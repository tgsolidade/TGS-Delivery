...
// ==========================================
// 🚀 4. ROBÔ DE NOTIFICAÇÃO PUSH (FCM) 🚀
// ==========================================
exports.notificarNovoPedido = onDocumentCreated("pedidos/{pedidoId}", async (event) => {
    ...
    try {
        ...
        const mensagem = {
            notification: {
                title: "⚡ NOVA ROTA TGS DELIVERY",
                body: `${pedido.lojaNome} - Taxa: ${pedido.taxa}`
            },
            android: {
                priority: "high" // <--- A CARTEIRADA PARA APP NATIVO
            },
            webpush: {
                headers: {
                    Urgency: "high" // <--- A CARTEIRADA PARA O PWA (WEB APP) ACORDAR
                }
            },
            tokens: tokens
        };
        ...
        console.log(`Push enviado! Sucesso: ${response.successCount}`);
...
exports.notificarTodosMotos = onRequest((request, response) => {
    cors(request, response, async () => {
        try {
            ...
            // Define o alvo da pesquisa no banco
            let alvo = 'motoboy';
            if (publico === 'lojas') {
                alvo = 'loja';
            }
            ...
            if (snapshot.empty) {
                return response.status(200).send({ sucesso: true, disparos: 0, msg: `Nenhum usuário online no alvo: ${alvo}` });
            }
            ...
            if (tokens.length === 0) {
                return response.status(200).send({ sucesso: true, disparos: 0, msg: "Sem tokens registrados" });
            }

            // Monta o pacote da notificação com as "Carteiradas"
            const payload = {
                notification: {
                    title: titulo,
                    body: mensagem
